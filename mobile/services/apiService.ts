...
export const getChatMessages = async (broadcastId: number, token?: string): Promise<ChatMessageDTO[] | { error: string }> => {
  try {
    ...
    const response = await fetchWithTimeout(`${API_BASE_URL}/chats/${broadcastId}`, {
      method: 'GET',
      headers,
    });
    ...
    try {
        ...
        return { error: `Failed to parse chat messages. Status: ${response.status}` };
    }
  } catch (error) {
...
export const getSongRequestsForBroadcast = async (broadcastId: number, token: string): Promise<SongRequestDTO[] | { error: string }> => {
  try {
    ...
    try {
        ...
        return { error: `Failed to parse song requests. Status: ${response.status}, Response: ${text}` };
    }
  } catch (error) {
...
export const getAllPollsForBroadcast = async (broadcastId: number, token: string): Promise<PollDTO[] | { error: string }> => {
  try {
    ...
    try {
        ...
        return { error: `Failed to parse polls. Status: ${response.status}` };
    }
  } catch (error) {
...
export const getActivePollsForBroadcast = async (broadcastId: number, token: string): Promise<PollDTO[] | { error: string }> => {
  try {
    const response = await fetchWithTimeout(`${API_BASE_URL}/polls/broadcast/${broadcastId}/active`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`,
    ...
    try {
        ...
        return { error: `Failed to parse active polls. Status: ${response.status}` };
    }
  } catch (error) {
...
export const hasUserVotedOnPoll = async (pollId: number, token: string): Promise<boolean | { error: string }> => {
  try {
    ...
    try {
        const hasVoted = JSON.parse(text);
        if (!response.ok) {
            // Try to parse error if possible, otherwise use status
            return { error: `Failed to check if user voted. Status: ${response.status}` };
        }
        return hasVoted as boolean;
    } catch (e) {
        return { error: `Failed to parse vote check. Status: ${response.status}` };
    }
  } catch (error) {
...
export const getCurrentActiveDJ = async (broadcastId: number, token: string): Promise<any | { error: string }> => {
  try {
    const response = await fetchWithTimeout(`${API_BASE_URL}/broadcasts/${broadcastId}/current-dj`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`,
    ...
    try {
        ...
        return { error: `Failed to parse active DJ. Status: ${response.status}` };
    }
  } catch (error) {
