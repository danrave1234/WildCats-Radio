...
export default function DJDashboard() {
  ...
  useEffect(() => {
    if (workflowState === WORKFLOW_STATES.STREAMING_LIVE && currentBroadcast) {
  ...
  }, [workflowState, currentBroadcast]);
  ...
  useEffect(() => {
    if (workflowState === WORKFLOW_STATES.STREAMING_LIVE && currentBroadcast) {
      ...
      // Setup WebSockets immediately when broadcast is live
      setupChatWebSocket();
      setupSongRequestWebSocket();
      setupPollWebSocket();

      return () => {
        console.log('DJ Dashboard: Cleaning up WebSocket connections');
